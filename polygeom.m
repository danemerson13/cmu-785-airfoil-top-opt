function [ geom, iner ] = polygeom( x, y ) 
...
xc = sum((x+xp).*a ,2) /6./A;
yc = sum((y+yp).*a ,2) /6./A;
Ixx = sum((y.*y +y.*yp + yp.*yp).*a, 2) /12;
Iyy = sum((x.*x +x.*xp + xp.*xp).*a, 2) /12;
Ixy = sum((x.*yp +2*x.*y +2*xp.*yp + xp.*y).*a, 2) /24;
 
dx = xp - x;
dy = yp - y;
P = sum(sqrt(dx.*dx +dy.*dy), 2);
